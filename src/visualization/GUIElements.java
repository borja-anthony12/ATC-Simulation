...
public class GUIElements extends JFrame {
	...
	public void startWindow() {
		window(this.windowWidth, this.windowHeight); // Calls the initializeWindow method and creates a window setting
														// the width
	...
	class AirportPanel extends JPanel {
		...
		public void drawPlanes() throws IOException {
			...
			for (PlaneAttributes plane : tower.getPlanes()) {
				...
				g2d.drawImage(plane.getPlaneImage(), (int) xPlane - (Image.getWidth() / 2),
						(int) yPlane - (Image.getHeight() / 2), null);
